                ...
                if($global:octo.ScanJobs.$($Title).Jobs[$i].Handle.IsCompleted -eq $True){
                    try{
                        if($global:octo.ScanJobs.$($Title).Jobs[$i].Thread.HadErrors){
                            Write-Host "$($global:octo.ScanJobs.$($Title).Jobs[$i].Target) has completed with errors"
                            $global:octo.ScanJobs.$($Title).Jobs[$i].Status = "Failed"
                        }else{
                            Write-Host "$($global:octo.ScanJobs.$($Title).Jobs[$i].Target) has completed with no errors :)"
